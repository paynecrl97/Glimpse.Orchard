...
namespace Glimpse.Orchard.PerfMon.Services {
    ...
    public class NewRelicPerformanceMonitor : DefaultPerformanceMonitor, IPerformanceMonitor {
        private readonly LazyField<IMessageBroker> _messageBroker;

        public NewRelicPerformanceMonitor()
        {
        ...
        public new void PublishMessage<T>(T message) {
            var durationMessage = message as IDurationMessage;
            if (durationMessage != null) {
                NewRelic.Api.Agent.NewRelic.RecordResponseTimeMetric("Custom/DEMO_Record_Response_Time_Metric", (long)durationMessage.Duration.TotalMilliseconds);
            }
        }
